...
let
  ...
  inherit (pkgsBuildHost) binutils cctools xcbuild;
in
self: super: {
  ...
  postInstall = super.postInstall or "" + ''
    ...
    for tool in lipo nm otool size strip; do
      if [ -e '${binutils.unwrapped}/bin/${stdenv.cc.targetPrefix}'$tool ]; then
        ln -s '${binutils.unwrapped}/bin/${stdenv.cc.targetPrefix}'$tool "$toolchainsPath/usr/bin/$tool"
      fi
    done
  '';
}
