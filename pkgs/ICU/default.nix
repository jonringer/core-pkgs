{
  ...
  buildPackages,
  fetchpatch2,
  icu, # The ICU version should correspond to the same one used by Apple’s ICU package
  mkAppleDerivation,
  python3,
  stdenvNoCC,
  testers,
...
let
  ...
  nativeBuildRoot = buildPackages.ICU.buildRootOnly;
  ...
  realAttrs = self: super: {
    ...
    postPatch = lib.optionalString self.finalPackage.doCheck ''
      ...
      tar -C "$NIX_BUILD_TOP/source" -axf ${lib.escapeShellArg icu.v78.src} icu/testdata
