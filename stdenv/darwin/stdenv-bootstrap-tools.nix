...
stdenv.mkDerivation (finalAttrs: {
  ...
  buildCommand =
    let
      ...
      curl_ =
        (curl.extendVariants {
          bootstrap = curl.minimal.variantArgs // {
            gssSupport = false;
            http2Support = false;
            scpSupport = false;
          };
    ...
    ''
      ...
      cp -rL ${getDev grep.pcre2}/include/* $out/include

      # Copy binutils.
      for i in as ld ar ranlib nm strip otool install_name_tool lipo codesign_allocate; do
        cp ${getBin binutils.unwrapped}/bin/$i $out/bin
      done
      ...
      cp -d ${getBin grep}/bin/grep $out/bin
      cp -d ${getBin make}/bin/* $out/bin
      cp -d ${getBin sed}/bin/* $out/bin
      cp -d ${getBin patch}/bin/* $out/bin

      cp -d ${getLib gettext}/lib/libintl*.dylib $out/lib
      cp -d ${getLib grep.pcre2}/lib/libpcre2*.dylib $out/lib
      ...
      cp -d ${getLib gmp.withCxx}/lib/libgmp*.dylib $out/lib
      ...
      # copy sigtool
      cp -d ${getBin sigtool}/bin/{codesign,sigtool} $out/bin
      ...
      cp ${getBin tar}/bin/tar unpack/bin
