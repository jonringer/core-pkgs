...
let
      ...
      thisStdenv = genericStdenv {
        ...
        preHook =
          ...
          + ''
            ...
            ${extraPreHook}
          ''
          + lib.optionalString (prevStage ? locale) ''
            export PATH_LOCALE=${prevStage.locale.data}/share/locale
          '';
        ...
        extraAttrs.fetchurlBoot = import ../../pkgs/fetchurl {
          inherit lib config;
          buildPackages.stdenvNoCC = prevStage.ccWrapperStdenv or thisStdenv;
          bootstrapCurl = bootstrapTools;
          cacert = null;
          fetchurl-bootstrap = null;
        };
        ...
        overrides =
          ...
          in
          builtins.removeAttrs stageOverrides [ "binutilsUnwrapped" ]
          // {
            binutils =
              ((stageOverrides.binutils or super.binutils).override {
                bintools = stageOverrides.binutilsUnwrapped or super.binutils.unwrapped;
              }).overrideAttrs
                (old: {
                  passthru = old.passthru // {
                    unwrapped = (stageOverrides.binutilsUnwrapped or super.binutils.unwrapped).overrideAttrs (oldRaw: {
                      passthru = oldRaw.passthru // {
                        real = throw "nothing in the Darwin bootstrap should depend on GNU binutils";
                        v2_44 = throw "nothing in the Darwin bootstrap should depend on GNU binutils";
                      };
                    });
                  };
                });
  ...
  darwinPackages = prevStage: { inherit (prevStage) sigtool; };
  darwinPackagesNoCC = prevStage: {
    binutilsUnwrapped = prevStage.binutils.unwrapped;
    localeData = prevStage.locale.data;
    inherit (prevStage)
      binutils
      libSystem
      ;
  ...
  disallowedPackages = prevStage: { inherit (prevStage) curl; };
  ...
  llvmLibrariesDarwinDepsNoCC = prevStage: { inherit (prevStage) libcxx; };
...
[
      ...
      overrides = self: super: {
        ...
        inherit
          ({
            ...
            libcxx = self.llvmPackages.libcxx;

            binutilsUnwrapped =
              (super.binutils.unwrapped.override { enableManpages = false; }).overrideAttrs
                (old: {
                  version = "boot";
            ...
            locale = super.locale.overrideAttrs (old: {
              passthru = old.passthru or { } // {
                data = self.stdenv.mkDerivation {
          ...
          selfLlvmPackages: _: {
            libclang = self.stdenv.mkDerivation {
              ...
              buildCommand = ''
                ...
                if (( $(ls "${bootstrapTools}/lib/clang" | wc -l) > 1 )); then
                  echo "Multiple LLVM versions were found at "${bootstrapTools}/lib/clang", but there must only be one used when building the stdenv." >&2
                  exit 1
                fi
            ...
            libllvm = self.stdenv.mkDerivation {
              ...
              buildCommand = ''
                ...
                ln -s $out $lib
                for tool in ${toString super.binutils.unwrapped.llvm_cmds}; do
                  cctoolsTool=''${tool//-/_}
  ...
  (
    ...
    assert allDeps isFromNixpkgs [
      ...
      {
        inherit (prevStage) binutils libSystem;
    ...
    stageFun prevStage {
      ...
      overrides = self: super: {
        ...
        cmake = super.cmake.minimal;

        # Use libiconv.real with gettext to break an infinite recursion.
        gettext = super.gettext.override { libiconv = super.libiconv.real; };

        # Disable grep’s tests for now due to impure locale updates in
        # macOS 15.4 breaking them in the bootstrap.
        grep = super.grep.overrideAttrs { doCheck = false; };
        ...
        inherit
          (
            ...
            // {
              ...
              signingUtils = prevStage.signingUtils.override { inherit (self) sigtool; };
              ...
              binutilsUnwrapped = super.binutils.unwrapped.override {
          ...
          (sdkPackagesNoCC prevStage)
          {
            ...
            inherit
              (
                ...
                // sdkDarwinPackages prevStage
                // {
                  inherit (prevStage) libSystem;
                  binutilsUnwrapped = super.binutils.unwrapped.override { enableManpages = false; };
                }
              )
              sigtool
              adv_cmds
      ...
      overrides =
        ...
        mergeDisjointAttrs [
          ...
          (llvmToolsDeps prevStage)
          {
            ...
            bashNonInteractive = super.bashNonInteractive.override { stdenv = self.bootstrapStdenv; };
            ...
            ncurses = super.ncurses.override {
              stdenv = self.bootstrapStdenv.override { shell = lib.getExe self.bashNonInteractive; };
            };
          ...
          (sdkPackages prevStage)
          {
            ...
            inherit
              (
                ...
                // sdkDarwinPackages prevStage
                // llvmLibrariesDarwinDepsNoCC prevStage
                # Rebuild binutils with the new LLVM, so only inherit libSystem from the previous stage.
                // {
                  inherit (prevStage) libSystem;
            ...
            inherit
              (
                ...
                // {
                  ...
                  locale = super.locale.overrideAttrs (old: {
                    passthru = old.passthru or { } // {
                      data = super.locale.data.override { inherit (prevStage) bmake; };
                    };
                  });
                }
              )
      ...
      stdenv = genericStdenv {
        ...
        overrides =
          ...
          mergeDisjointAttrs [
            ...
            (sdkPackagesNoCC prevStage)
            {
              ...
              inherit
                (
                  ...
                  // llvmLibrariesDarwinDepsNoCC prevStage
                  // {
                    inherit (prevStage) libSystem sigtool;
                    locale = super.locale.overrideAttrs (old: {
                      passthru = old.passthru or { } // {
                        data = prevStage.locale.data;
                      };
                    });
                  }
                )
            ...
            # binutils is not yet ready to be target-independent.
  ...
  (
    ...
    assert isBuiltByNixpkgsCompiler prevStage.sigtool;

    assert isFromNixpkgs prevStage.libSystem;
    assert isFromNixpkgs prevStage.libc;
    assert isFromNixpkgs prevStage.binutils.unwrapped;
    ...
    assert isBuiltByNixpkgsCompiler prevStage.binutils.unwrapped.real;
    assert isFromNixpkgs prevStage.binutils.unwrapped.real.src;
